...
class TestVideoToFrames:
    ...
    def run(self,path,name,mode,frames,set):
        ...
        catalog = filename.split("-")[1] + "-" + filename.split("-")[2]  #获取中间目录名nm-01
        ...
        try :
            ...
            # print("id2",id2)
            # print("id3",id3)
            os.makedirs("dataset/test_probe/"+str(id2))

        except:
            print("diractory maked")
        list = os.listdir("dataset/test_probe/"+id2)
        seq = len(list)+1
        try:
            os.makedirs("dataset/test_probe/"+str(id2)+"/"+catalog.format(seq)+"/"+angle)
        ...
        while (video.isOpened()):
            ...
            if image is not None:
                ...
                if( images is not None ):
                    ...
                    if i >1:#跳过前n张
                        filename = "dataset/test_probe/{0:03}".format(id)+"/"+catalog+"/"+angle+ "/{0:03}-".format(id)+catalog+"-"+angle+"-"+"{0:03}".format(i-4) + ".png"
        ...
        # fw.close()
        #删除空白帧
        listd=os.listdir("dataset/test_probe/"+str(id2)+"/"+catalog+"/"+angle)
...
if __name__ == '__main__':
    f=TestVideoToFrames()
    path=r"data/009-nm-01-090.mp4"
    name="XXX"
    f.run(path,name,"noknn",50,[2,2,2])
